...
async def retrieve_sensors():
    sensors = []
    async for sensor in sensor_collection.aggregate([
        {'$sort': {'updated': -1}},
            {'$group': {'_id': {'device_id': '$device_id'}, "doc": {"$first": "$$ROOT"}}}]):
        sensors.append(sensor_helper(sensor['doc']))
    return sensors
...
async def retrieve_sensor(device_id: int) -> dict:
    sensors = []
    async for sensor in sensor_collection.aggregate([
        {'$match': {"device_id": int(device_id)}},
        {'$sort': {'updated': -1}},
            {'$group': {'_id': {'device_id': '$device_id'}, "doc": {"$first": "$$ROOT"}}}]):
        sensors.append(sensor_helper(sensor['doc']))
    return sensors
