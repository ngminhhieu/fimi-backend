...
async def get_sensor_data(sensor: SensorSchema = Body(...)):
    sensor = jsonable_encoder(sensor)
    updated_sensor = await update_sensor(sensor["device_id"], sensor)
    if sensor:
        return ResponseModel(updated_sensor, "sensor updated successfully.")
    return ErrorResponseModel(
        "An error occurred", 404, "sensor with device_id {0} doesn't exist".format(sensor["device_id"])
    )
